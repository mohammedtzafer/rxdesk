...
export default function DrugRepVisitDetailPage() {
  ...
  if (error || !visit) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <h2 className="text-[21px] font-bold text-foreground">Visit not found</h2>
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {/* Visit header card */}
      <div className="bg-card rounded-xl p-6 border border-[rgba(0,0,0,0.06)]">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h1 className="text-[28px] sm:text-[32px] font-semibold leading-[1.1] tracking-tight text-foreground">
                {visit.drugRep.firstName} {visit.drugRep.lastName}
              </h1>
              <p className="mt-0.5 text-[17px] text-muted-foreground">{visit.drugRep.company}</p>
            </div>
          </div>
          <div className="flex flex-col gap-2 text-[14px] text-muted-foreground sm:text-right">
          ...
          <div className="mt-4 pt-4 border-t border-border">
            ...
            <div className="flex flex-wrap gap-2">
              {visit.samplesLeft.map((sample, i) => (
                <div
                  key={i}
                  className="inline-flex items-center gap-2 px-3 py-1.5 bg-muted rounded-lg text-[13px]"
                >
      ...
      {visit.providerDetails.length > 0 && (
        <div>
          <h2 className="text-[21px] font-semibold text-foreground mb-3">
          ...
          <div className="space-y-4">
            {visit.providerDetails.map((pd) => {
              ...
              return (
                <div
                  key={pd.provider.id}
                  className="bg-card rounded-xl border border-[rgba(0,0,0,0.06)] overflow-hidden"
                >
                  {/* Provider header */}
                  <div className="px-5 py-4 border-b border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[12px] text-muted-foreground/60 mt-0.5">{sinceLabel}</p>
                      </div>
                    </div>
                    <div className="flex flex-col items-start sm:items-end shrink-0">
                      ...
                      <span className="text-[12px] text-muted-foreground mt-1">total Rx filled</span>
                    </div>
                  </div>
                  ...
                  {pd.rxByDrug.length > 0 ? (
                      ...
                      <table className="w-full min-w-[500px]">
                        <thead>
                          <tr className="border-b border-[rgba(0,0,0,0.04)]">
                            <th className="text-left px-5 py-2.5 text-[11px] font-semibold text-muted-foreground/60 uppercase tracking-wide">
                              Drug
                            </th>
                            <th className="text-right px-5 py-2.5 text-[11px] font-semibold text-muted-foreground/60 uppercase tracking-wide">
                              Fills
                            </th>
                            <th className="text-right px-5 py-2.5 text-[11px] font-semibold text-muted-foreground/60 uppercase tracking-wide">
                              Total Qty
                            </th>
                            <th className="px-5 py-2.5 text-[11px] font-semibold text-muted-foreground/60 uppercase tracking-wide">
                              Type
                            </th>
                          </tr>
                        </thead>
                          ...
                          {pd.rxByDrug.map((drug, i) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-5 py-2.5 text-right text-[14px] font-semibold text-foreground">
                                  {drug.count.toLocaleString()}
                                </td>
                                <td className="px-5 py-2.5 text-right text-[14px] text-muted-foreground">
                    ...
                    <div className="px-5 py-6 text-center text-[14px] text-muted-foreground/60">
      ...
      {visit.providerDetails.length === 0 && (
        <div className="bg-card rounded-xl p-12 text-center border border-[rgba(0,0,0,0.06)]">
          <User className="w-12 h-12 mx-auto text-muted-foreground/30" />
          <h3 className="mt-4 text-[17px] font-semibold text-foreground">No providers linked</h3>
