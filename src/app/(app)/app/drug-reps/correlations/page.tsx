    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[40px] font-semibold leading-[1.1] tracking-tight text-foreground">
            Drug Rep Visit Correlations
          </h1>
          <p className="mt-1 text-[17px] text-muted-foreground">
            Rx volume changes within 4 weeks of drug rep visits to provider offices
          </p>
        </div>
        <select value={days} onChange={(e) => setDays(parseInt(e.target.value))} className="h-9 rounded-lg border border-border px-3 text-[14px] bg-card">
      ...
      {loading ? (
        <div className="mt-8 space-y-4">
          {[1, 2, 3].map((i) => <div key={i} className="bg-card rounded-lg p-5 h-24 animate-pulse" />)}
        </div>
      ) : significantCorrelations.length === 0 ? (
        <div className="mt-8 bg-card rounded-xl p-12 text-center">
          <AlertTriangle className="w-12 h-12 mx-auto text-muted-foreground/30" />
          <h2 className="mt-4 text-[21px] font-bold text-foreground">No significant correlations</h2>
        ...
        <div className="mt-6 space-y-3">
          {significantCorrelations.map((c, i) => (
            <div key={`${c.visitId}-${c.providerId}-${i}`} className="bg-card rounded-xl p-5">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-[14px] font-medium text-foreground">
                    {c.repName} <span className="text-muted-foreground">({c.company})</span>
                  </p>
              ...
              <div className="mt-3 flex gap-6 text-[12px]">
                <div>
                  <span className="text-muted-foreground">Pre-visit (4 wks):</span>{" "}
                  <span className="font-medium text-foreground">{c.preVisitVolume} Rx</span>
                </div>
                <div>
                  <span className="text-muted-foreground">Post-visit (4 wks):</span>{" "}
                  <span className="font-medium text-foreground">{c.postVisitVolume} Rx</span>
                </div>
              </div>
